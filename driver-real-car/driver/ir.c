...
#define GPIOPIN_LEFT 			9 // GPIO PIN 
...
int deviceArray [ DEVICE_NUM ] = { LEFT_IR, RIGHT_IR} ;
char  * names 	[ DEVICE_NUM ] = { "LEFT_IR", "RIGHT_IR" } ;
char * devnames [ DEVICE_NUM ] = { "car/left_ir", "car/right_ir" } ;
int minorNumber [ DEVICE_NUM ] ;
char * classArr [ DEVICE_NUM ] = { "irClass1", "irClass2" } ;
char * region   [ DEVICE_NUM ] = { "LEFT_IR_REGION", "RIGHT_IR_REGION2" } ;
...
static ssize_t DeviceReadLeft ( struct file * file, char __user * buf, size_t len, loff_t * off )
{
    ...
    uint32_t gpioState = gpio_get_value ( LEFT_IR) ;
    uint32_t value;

    value = ( gpioState == IR_DETECTED ) ? OBJECT_FOUND :  OBJECT_NOT_FOUND;

    if ( copy_to_user ( buf, & value, sizeof ( uint32_t ) ) > 0 ) {
...
static ssize_t DeviceReadRight ( struct file * file, char __user * buf, size_t len, loff_t * off )
{
    ...
    uint32_t gpioState = gpio_get_value ( RIGHT_IR) ;
    uint32_t value;
